...
function setup() {
  ...
  ibmcloud wsk package create ${FN_PRIVATE_PACKAGE}
  ...
  _out Deploying function: ${FN_PRIVATE_PACKAGE}/function-protected
  ibmcloud wsk action create ${FN_PRIVATE_PACKAGE}/function-protected ${root_folder}/../function-protected/function-protected.js --kind nodejs:10 -a web-export true -p config "${CONFIG}"
  ...
  SWAGGER_TEMPLATE_PATH="${root_folder}/../function-protected/swagger-template.json"
  SWAGGER_REAL_PATH="${root_folder}/../function-protected/swagger-protected.json"
  [[ -e "${SWAGGER_REAL_PATH}" ]] && rm SWAGGER_REAL_PATH
  cp ${SWAGGER_TEMPLATE_PATH} ${SWAGGER_REAL_PATH}
  
  readonly ACTION_NAMESPACE_AND_PACKAGE=$( ibmcloud fn action get ${FN_PRIVATE_PACKAGE}/function-protected | awk '/namespace/{print $2}' | sed "s/,//" | sed "s/\"//g" )
  ...
  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${SWAGGER_REAL_PATH} xxx-namespace-xxx ${ACTION_NAMESPACE}
  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${SWAGGER_REAL_PATH} xxx-action-url-xxx ${ACTION_URL}
  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${SWAGGER_REAL_PATH} xxx-tenantid-xxx ${APPID_TENANTID}
  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${SWAGGER_REAL_PATH} xxx-sample-package-xxx ${FN_PRIVATE_PACKAGE}
  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${SWAGGER_REAL_PATH} xxx-api-basepath-xxx $HIDDEN_API_BASEPATH
...
export APPID_SECRET CLOUDANT_USERNAME CLOUDANT_PASSWORD IBMCLOUD_SPACE
