...
export class HomeComponent {
  ...
  onButtonInvokeActionClicked1(): void {
    ...
    this.httpClient.get(this.protectedUrl+"/getMorePeople", options)
  ...
  onButtonInvokeActionClicked2(): void {
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer ' + this.accessToken,
    ...
    const options = { headers: headers };
    this.httpClient.get(this.protectedUrl+"/getPeople", options)
      .subscribe(result => this.resultOfProtectedAPI = JSON.stringify(result, null, 2), 
                 error  => this.resultOfProtectedAPI = JSON.stringify(error , null, 2)
                 );
  }
