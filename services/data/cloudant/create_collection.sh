...
python3 create_collection.py "${CLOUDANT_DB_NAME}"
if [ $? -ne 0 ]; then
    _fatal "Failed to create data collection in '${CLOUDANT_DB_NAME}'"
else
    _ok "Successfully created data collection in '${CLOUDANT_DB_NAME}'"
fi